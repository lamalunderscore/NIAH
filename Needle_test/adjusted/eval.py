...
class TokenBucket:
    ...
    def _refill(self):
        ...
        self.tokens = min(
            self.capacity, self.tokens + (time_passed * self.capacity / 60)
        )
        self.last_updated = now

        # Remove token requests older than 1 minute
        while self.token_requests and self.token_requests[0] < now - timedelta(
            minutes=1
        ):
...
class RateLimitedOpenAI:
    ...
    def pred_openai(
        self, model_name, messages, max_retries=5, backoff_factor=2
    ):
        ...
        for attempt in range(max_retries):
            ...
            try:
                ...
                print(
                    f'Error Occurs: "{str(e)}"        Retry {attempt + 1}/{max_retries} ...'
                )
...
if __name__ == "__main__":
    ...
    # assumes that the prediction directories start with "pred"
    directories = [
        os.getcwd() + "/LongAlign/Needle_test/" + d
        for d in os.listdir("LongAlign/Needle_test/")
        if os.path.isdir(os.getcwd() + "/LongAlign/Needle_test/" + d)
        and d.startswith("pred")
    ]
    ...
    for d in directories:
        print(d)
        if d[-1].isdigit():
            save_name = f"{model_provider}_{model_name}_eval_k{d[-1]}.json"
        else:
            save_name = f"{model_provider}_{model_name}_eval.json"
        ...
        for filename in os.listdir(d):
            ...
            if model_provider == "OpenAI":
                ...
                raise NotImplementedError(
                    f"Not implemented model provider: {model_provider}"
                )
